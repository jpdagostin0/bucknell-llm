...
SECRET_DIR_MARKERS = {("tools", "moodle-dl", "state"), ("tools", "google-auth", "state")}
...
def assert_not_secret(path: Path) -> None:
    ...
    if "state" in parts and ("moodle-dl" in parts or "google-auth" in parts):
...
def catalog() -> dict[str, Any]:
    return {
        ...
        "invoke": [
            ...
            r".\tools\read-file-lines\read-file-lines.ps1 <path> --start 1 --end 40",
            "python tools/read-file-lines/read_file_lines.py <path> [start] [end]",
        ],
        "shell": "powershell",
        "usage": {
            "read": "<path> [start] [end]   # 1-based inclusive line numbers",
        },
