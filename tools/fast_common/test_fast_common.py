...
class FastCommonTests(unittest.TestCase):
    ...
    def test_repeated_class_flags_build_a_list(self) -> None:
    ...
    def test_json_is_rejected_for_fast_runners(self) -> None:
        with self.assertRaises(fc.ToolError) as raised:
            fc.parse_invocation(
                ["run", "--json", '{"class":"MATH-245"}'],
                default_command="run",
            )
    ...
    def test_sandbox_and_scratchpad_paths_are_rejected(self) -> None:
        self.assertTrue(fc.is_sandbox_or_scratchpad_path("/inputs/tool.py"))
        self.assertTrue(fc.is_sandbox_or_scratchpad_path("/outputs/due_work.json"))
        self.assertTrue(
            fc.is_sandbox_or_scratchpad_path(
                r"C:\Users\jpdagostino\.lmstudio\scratchpads\2w\due_work.json"
            )
        )
        self.assertFalse(fc.is_sandbox_or_scratchpad_path(r".\payload.json"))
        with self.assertRaises(fc.ToolError) as raised:
            fc.parse_invocation(
                ["run", "--json-file", "/inputs/payload.json"],
                default_command="run",
            )
        self.assertIn("Never pass --json", raised.exception.message)

    def test_missing_host_json_file_is_usage_error(self) -> None:
        with self.assertRaises(fc.ToolError) as raised:
            fc.parse_invocation(
                ["run", "--json-file", r".\missing-payload.json"],
                default_command="run",
            )
        self.assertIn("Never pass --json", raised.exception.message)

    def test_emulated_python_is_rejected(self) -> None:
        self.assertFalse(fc.is_emulated_python())
        with unittest.mock.patch.object(fc.sys, "platform", "emscripten"):
            with self.assertRaises(fc.ToolError) as raised:
                fc.require_host_python()
        self.assertIn("in-memory Python sandbox", raised.exception.message)
    ...
    def test_needs_llm_has_next_action(self) -> None:
        asked = fc.needs_llm("missing_due", issue="JPS-12")
        self.assertEqual(asked["kind"], "missing_due")
        self.assertEqual(asked["action"], "ask_user")
        self.assertEqual(asked["issue"], "JPS-12")
        ran = fc.needs_llm(
            "pending_create",
            command='python tools/run_tool/run_tool.py fast_linear save --apply',
        )
        self.assertEqual(ran["action"], "run")
        self.assertIn("fast_linear save --apply", ran["command"])
...
class LinearApplyTests(unittest.TestCase):
    ...
    def test_issue_title_uses_retained_name_and_matches_existing(self) -> None:
        ...
        explicit = issue_title(
            course,
            {
                "title": "Assignment #3",
                "matched": "attachments/MATH-212 Assignment-03.pdf",
            },
        )
        self.assertEqual(explicit, "MATH 212 — Assignment #3")
    ...
    def test_pymarkdown_argv_uses_module_not_trampoline(self) -> None:
        command = fc.pymarkdown_argv("scan", ".")
        self.assertEqual(Path(command[0]).name, "python.exe")
        self.assertEqual(command[1:3], ["-m", "pymarkdown"])
        self.assertTrue(all("pymarkdown.exe" not in part for part in command))
