    ...
    if not status.get("authorized"):
        return {
            ...
            "needs_user": [
                needs_llm(
                    "google_auth",
                    command="python tools/run_tool/run_tool.py google_auth login",
...
def process_course(course: dict[str, Any], payload: dict[str, Any]) -> dict[str, Any]:
    ...
    for file_obj in drive.get("not_in_moodle") or []:
        ...
        if dest.exists():
            continue
        if apply_changes:
            staging = vault_root() / "tools/fast_google_drive/output" / suggested
            try:
                download_drive_file(
                    str(file_obj["id"]),
                    staging,
...
def main() -> int:
    return run_cli("fast_sync_class", {"run": run})
