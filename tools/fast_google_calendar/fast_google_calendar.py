...
def require_auth() -> dict[str, Any]:
    status = google_status("google_calendar")
    if not status.get("authorized"):
        raise ToolError(
            r"Calendar is not authorized. Run python tools/run_tool/run_tool.py google_auth login",
            "needs_auth",
        )
    return status
...
def upcoming(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    data = run_json_tool("google_calendar", args)
    return {
        "days": data.get("days"),
...
def create(payload: dict[str, Any]) -> dict[str, Any]:
    require_auth()
    return run_json_tool("google_calendar", ["create_event", *payload_flags(payload)])


def update(payload: dict[str, Any]) -> dict[str, Any]:
    require_auth()
    return run_json_tool("google_calendar", ["update_event", *payload_flags(payload)])


def delete(payload: dict[str, Any]) -> dict[str, Any]:
    require_auth()
    return run_json_tool("google_calendar", ["delete_event", *payload_flags(payload)])


def respond(payload: dict[str, Any]) -> dict[str, Any]:
    require_auth()
    return run_json_tool("google_calendar", ["respond_to_event", *payload_flags(payload)])
