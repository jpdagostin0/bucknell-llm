...
_FAST_COMMON = Path(__file__).resolve().parents[1] / "fast_common"
...
def fetch_dashboard(payload: dict[str, Any]) -> dict[str, Any]:
    for key in ("output", "out", "write", "save"):
        if payload.get(key):
            raise ToolError(
                f"Do not pass --{key}. fast_dashboard prints JSON or markdown on stdout; "
                "do not write due_work.json to .lmstudio/scratchpads, C:/tmp, /inputs, or /outputs.",
                "usage",
            )
