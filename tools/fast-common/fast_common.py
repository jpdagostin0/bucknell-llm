...
def parse_invocation(
    ...
    elif not sys.stdin.isatty() and not unknown:
...
def powershell() -> str:
    return os.environ.get("POWERSHELL", "powershell.exe")


def powershell_file_args(args: list[str]) -> list[str]:
    """Quote JSON payloads so PowerShell -File does not treat them as expressions."""
    quoted: list[str] = []
    for value in args:
...
def run_ps1(
    ...
    *,
    ...
    if check and result.returncode != 0:
        ...
        raise ToolError(
            f"{script.name} failed ({result.returncode}): {detail[:800]}",
            "tool",
        )
    return result
...
def tool_sentinels() -> dict[str, Path]:
    ...
    return {
        ...
        "moodle-dl": root / "tools/moodle-dl/.venv/Scripts/moodle-dl.exe",
        "markitdown": root / "tools/markitdown/.venv/Scripts/markitdown.exe",
        "pypdf": root / "tools/pypdf/.venv/Scripts/python.exe",
        "google-auth": root / "tools/google-auth/.venv/Scripts/python.exe",
        "google-drive": root / "tools/google-drive/.venv/Scripts/python.exe",
        "gmail": root / "tools/gmail/.venv/Scripts/python.exe",
        "google-calendar": root / "tools/google-calendar/.venv/Scripts/python.exe",
        ...
        "ls-lint": root / "tools/ls-lint/bin/ls-lint-windows-amd64.exe",
    }
...
def ensure_tool(name: str) -> Path:
    ...
    if sentinel.exists():
        return sentinel
    installer = vault_root() / "tools" / name / "install.ps1"
    if name in {"google-drive", "gmail", "google-calendar"}:
        ensure_tool("google-auth")
    if not installer.exists():
        raise ToolError(f"Missing installer for {name}.", "tool")
    run_ps1(installer, timeout=600, check=True)
...
def run_json_tool(
    ...
    *,
    ...
    ensure_tool(name)
    script = vault_root() / "tools" / name / f"{name}.ps1"
    result = run_ps1(script, args, timeout=timeout)
...
def moodle_state_dir() -> Path:
    return vault_root() / "tools/moodle-dl/state"
...
def convert_pdf(source: Path, output_name: str | None = None) -> dict[str, Any]:
    ...
    if destination.exists():
        destination.unlink()
    script = vault_root() / "tools/markitdown/markitdown.ps1"
    run_ps1(
        script,
...
def list_pdf_labels(textbook: Path) -> list[dict[str, str]]:
    ensure_tool("pypdf")
    script = vault_root() / "tools/pypdf/extract-pages.ps1"
    result = run_ps1(script, [str(textbook), "--list-labels"], timeout=60, check=True)
...
def list_drive_children(folder_id: str) -> list[dict[str, Any]]:
    ...
    while True:
        ...
        data = run_json_tool("google-drive", args)
...
def run_moodle_sync(*, skip_download: bool = False) -> dict[str, Any]:
    ...
    script = vault_root() / "tools/moodle-dl/moodle-dl.ps1"
    result = run_ps1(script, timeout=1800)
    return {
...
def run_vault_checks() -> dict[str, Any]:
    ...
    pymarkdown = root / "tools/pymarkdown/.venv/Scripts/pymarkdown.exe"
    record(
        "pymarkdown",
        subprocess.run(
            [str(pymarkdown), "--config", str(root / ".pymarkdown.yml"), "scan", "--recurse", "--respect-gitignore", "."],
            cwd=root,
            capture_output=True,
            text=True,
            encoding="utf-8",
    ...
    validator = subprocess.run(
        [str(python), str(root / "tools/vault-lint/validate_vault.py"), "."],
        cwd=root,
        capture_output=True,
        text=True,
        encoding="utf-8",
    ...
    tests = subprocess.run(
        [str(python), str(root / "tools/vault-lint/test_validate_vault.py")],
        cwd=root,
        capture_output=True,
        text=True,
        encoding="utf-8",
    ...
    ls_lint = root / "tools/ls-lint/bin/ls-lint-windows-amd64.exe"
    record(
        "ls_lint",
        subprocess.run(
...
def autofix_markdown() -> dict[str, Any]:
    ensure_tool("pymarkdown")
    script = vault_root() / "tools/vault-lint/autofix.ps1"
    result = run_ps1(script, timeout=180)
    return {
