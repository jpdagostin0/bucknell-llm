...
def require_auth() -> dict[str, Any]:
    ...
    if not status.get("authorized"):
        raise ToolError(
            r"Drive is not authorized. Run .\tools\google-auth\google-auth.ps1 login",
            "needs_auth",
        )
    return status
...
def run(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    recent = run_json_tool("google-drive", ["list_recent_files", "--pageSize", "10"])
...
def download(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    staging = vault_root() / "tools/google-drive/output" / title
...
def main() -> int:
    return run_cli("fast-google-drive", {"run": run, "download": download})
