...
def run(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    if apply_changes and (
        get_value(payload, "issues")
        or get_value(payload, "title")
        or get_value(payload, "file", "files")
    ):
        created_issues = save_course_issues(
            payload, workspace, apply=True
        ).get("results") or []
    ...
    if as_list(get_value(payload, "issues")) and not apply_changes:
        remaining.append(
            needs_llm(
                "pending_create",
                message="Pass --apply to create the supplied Linear issues.",
                count=len(as_list(get_value(payload, "issues"))),
            )
        )
...
def main() -> int:
    return run_cli("fast_linear_sync", {"run": run})
